...
function VehicleCard({ vehicle }: { vehicle: Vehicle }) {
  const grad = GRADIENTS[parseInt(vehicle._id, 10) % GRADIENTS.length] || GRADIENTS[0];
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        ...
        (e.currentTarget as HTMLElement).style.boxShadow = "0 12px 40px rgba(124,58,237,0.2)";
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      <div
        style={{
          height: "160px",
          background: vehicle.images?.[0] ? `url(${vehicle.images[0]}) center/cover` : grad,
          display: "flex",
          alignItems: "center",
      ...
      >
        {!vehicle.images?.[0] && <Car size={56} color="rgba(255,255,255,0.2)" />}
        <div
          style={{
            position: "absolute",
            top: "0.75rem",
        ...
        <div style={{ position: "absolute", bottom: "0.75rem", left: "0.75rem" }}>
          <span className={`badge ${vehicle.fuelType === "electric" ? "badge-success" : "badge-info"}`}>
            {vehicle.fuelType}
          </span>
        </div>
      ...
      <div style={{ padding: "1.25rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem" }}>
          <div>
            <h3 style={{ fontSize: "0.9375rem", fontWeight: 700, color: "white" }}>
...
function SkeletonCard() {
  return (
    <div className="card" style={{ overflow: "hidden", padding: 0 }}>
      ...
      <div style={{ padding: "1.25rem" }}>
        <div style={{ height: "16px", background: "rgba(255,255,255,0.06)", borderRadius: "0.5rem", marginBottom: "0.5rem", width: "65%" }} />
        <div style={{ height: "12px", background: "rgba(255,255,255,0.04)", borderRadius: "0.375rem", marginBottom: "1rem", width: "45%" }} />
        <div style={{ height: "12px", background: "rgba(255,255,255,0.04)", borderRadius: "0.375rem", marginBottom: "1rem", width: "80%" }} />
        <div style={{ height: "34px", background: "rgba(124,58,237,0.1)", borderRadius: "0.75rem" }} />
      </div>
    </div>
  );
}
...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const updates: Partial<VehicleFilters> = {};
      if (params.get("location")) updates.location = params.get("location")!;
      if (params.get("startDate")) setSearchInput(params.get("startDate")!);
    ...
    >
      <div style={{ maxWidth: "1280px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "2rem" }}>
          <h1
            style={{ fontSize: "1.875rem", fontWeight: 800, color: "white", marginBottom: "0.5rem" }}
          >
            Browse Vehicles
          </h1>
          <p style={{ color: "var(--text-secondary)", fontSize: "0.875rem" }}>
            {isLoading ? "Loading..." : `${total} vehicle${total !== 1 ? "s" : ""} available`}
          </p>
        </div>
                ...
                onChange={(e) =>
                  setFilters((f) => ({
                    ...
                    fuelType:
                      e.target.value === "All"
                        ? undefined
                        : (e.target.value as VehicleFilters["fuelType"]),
                    page: 1,
                  }))
                }
              >
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: "center", padding: "5rem 1rem" }}>
            ...
            <h3 style={{ color: "white", fontWeight: 700, marginBottom: "0.5rem" }}>
              No vehicles found
            </h3>
